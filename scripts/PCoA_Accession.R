...
#BDiversidad
tmp <-read.delim("OTU_table_spermosphere", h = T)
head(tmp)
c <- tmp[,2:37]
head(c)
...
endo_total.ord       <- ordinate(biom_modified, "PCoA", "bray", ~ Accession)
...
all_plot_2       = endo_plot  + geom_point(aes(shape=Accession),   
                                           size = 4) + geom_text(aes(label = Accession), size=0) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
adonis(Distance ~ Accession, as(sample_data(endo_metg), "data.frame"))
