...
endo_total.ord       <- ordinate(biom_modified, "NMDS", "manhattan", ~ Group)
...
all_plot_2       = endo_plot  + geom_point(aes(shape=Group),   
                                           size = 4) + geom_text(aes(label = Group), size=0) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
adonis(Distance ~ Group, as(sample_data(endo_metg), "data.frame"))
